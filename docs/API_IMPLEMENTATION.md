...
```json
{
  ...
  "orderId": "pass-order-20260714-001",
  "amount": 9900
}
```
**201 응답**
```json
{
  ...
  "expires_at": "2025-08-06T23:59:59Z",
  "paid": 9900,
  "payment_key": "토스에서_받은_paymentKey",
  "order_id": "pass-order-20260714-001",
  "payment_status": "DONE",
  "message": "패스 구매가 완료됐어요"
}
```
> 서버가 `amount`를 해당 패스의 가격과 비교한 뒤 토스 승인 API를 호출한다. 승인 성공 후에만 UserPass와 `pass_purchase` Transaction을 생성한다.
...
### 8-7. `POST /checkout` — 토스 결제 승인 및 체크아웃 확정 (Image 5)
프론트엔드에서 토스 결제 인증이 끝난 뒤 호출한다. 서버가 혜택을 다시 계산하고, 전달받은 최종 결제금액이 일치할 때만 토스 승인 API를 호출한다.
...
```json
{
  ...
  "method": "easy_pay",
  "paymentKey": "토스에서_받은_paymentKey",
  "orderId": "checkout-order-20260714-001",
  "payment_amount": 16200
}
```
```
store_id       int
amount         int       // 할인 전 주문금액
benefit_id     string    // /checkout/benefits에서 고른 값. "none"이면 원가
method         string    // 결제수단
paymentKey     string?   // 토스 인증 성공 후 받은 결제키
orderId        string?   // 결제 요청 때 사용한 고유 주문번호
payment_amount int?      // 토스에 승인 요청할 최종 결제금액
```

> 서버가 `final_amount = amount - discount`를 다시 계산한다. `final_amount > 0`이면
> `paymentKey`, `orderId`, `payment_amount`가 모두 필요하고 `payment_amount`는
> `final_amount`와 같아야 한다. `final_amount == 0`인 전액 할인 결제는 토스 승인 없이
> 로컬 결제 및 혜택 사용 내역만 저장한다.
...
```json
{
  ...
  "payment_id": 101,
  "payment_key": "토스에서_받은_paymentKey",
  "order_id": "checkout-order-20260714-001",
  "payment_status": "DONE",
...
payment_id       int      // 로컬 payments 레코드 ID
payment_key      string   // 승인 완료된 토스 paymentKey
order_id         string   // 승인 완료된 주문번호
payment_status   string   // 정상 확정 시 DONE
message          string
```
> `benefit_kind`가 coupon이면 "한 번 쓰면 사라지는 소진형" 안내, pass면 "이용 기록으로 남아" 안내 (Image 5의 3·4번째 차이).
> 토스 승인 성공 시 Payment/TossPayment를 저장하고 Transaction에 `coupon_use` 또는 `pass_use`를 기록한다. 동일한 paymentKey+orderId 재요청은 저장된 성공 응답을 반환하며, 둘 중 하나만 재사용하면 409로 차단한다.

**400 — 서버 계산 금액과 불일치**
```json
{ "error": "payment_amount_mismatch", "message": "결제 금액이 서버에서 계산한 최종 금액과 일치하지 않아요" }
```

**토스 승인 실패**는 토스의 오류 상태를 전달하고, 네트워크 오류는 500으로 반환한다. 어떤 경우에도 로컬 쿠폰·패스·결제 상태는 변경하지 않는다.

**환경변수**

- `TOSS_SECRET_KEY`: 운영에서는 토스 개발자센터의 라이브 시크릿 키를 설정한다.
- 미설정 시 문서용 테스트 키 `test_gsk_docs_OaPz8L5KdmLxqZqRxTmwrlBN`을 사용한다.
- 승인 URL: `POST https://api.tosspayments.com/v1/payments/confirm`
...
```
> 이후 손님 앱은 이 `store_id`·`amount`로 `GET /checkout/benefits`(8-6)를 호출해 혜택을 고른다. 토스 인증 후 `POST /checkout`의 `order_amount`에는 QR 금액, `amount`에는 할인 적용 후 최종 승인금액을 넣는다.
...
- **`type == "payment"`**: 즉시 QR을 `CONSUMED`로 바꾸고 아래를 반환. 이후 손님 앱은 이 `store_id`/`amount`로 `GET /checkout/benefits`를 호출하고, 토스 인증 성공 뒤 승인정보를 포함해 `POST /checkout`을 호출한다.
  ```json
...
> 기존 8-6 `GET /checkout/benefits`는 그대로 유효하다. `POST /scan`이 `kind=payment`를 반환하면 QR의 금액을 `order_amount`로 유지해 혜택을 계산하고, 토스 인증 뒤 최종 `amount`와 승인정보로 8-7 `POST /checkout`을 호출한다.
